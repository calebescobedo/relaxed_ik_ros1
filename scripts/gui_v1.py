...
class App(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        self.gohead_pub = rospy.Publisher("/gui_pub", GUIMsg, queue_size=1)
        self.text_overlay_pub = rospy.Publisher("/text_overlay", GUIMsg, queue_size=1)
        self.xbox_sub = rospy.Subscriber("/gui_done", GUIMsg, self.gui_done_cb)
        script_dir = os.path.dirname(__file__)
        theme_path = os.path.join(script_dir, 'Hades.json')
        customtkinter.set_default_color_theme(theme_path)
        ...
        self.current_transfer_number = 0
        self.total_transfers = 0
        self.transfer_times = []
        self.start_flag = ""
        self.done_flag = ""

        # Publish the text overlay message immediately after the window is created
        self.display_text_overlay_message()
        ...
        self.input_frame = None
        self.equal_transfers = False
        self.custom_transfers = False
    ...
    def start_button_cb(self):
        ...
        # Proceed to show the input form
        self.show_input_form()

    def display_text_overlay_message(self):
        # Create and populate GUIMsg message
        overlay_msg = GUIMsg()
        overlay_msg.go_flag = ""  
        overlay_msg.num_transfers = self.total_transfers
        overlay_msg.curr_transfers = self.current_transfer_number
        overlay_msg.transfer_times = ', '.join(self.transfer_times)
        overlay_msg.done_flag = self.done_flag
        overlay_msg.test_msg = "" 
        overlay_msg.marker_count = 0  # Update this as needed
        # overlay_msg.robot_mode = 

        self.text_overlay_pub.publish(overlay_msg)
    ...
    def handle_confirm(self):
        ...
        if choice not in ['equal', 'custom']:
            self.show_error_message("Invalid choice! Please enter 'equal' or 'custom'.")
            return
        if choice == "equal": self.equal_transfers = True
        else: self.custom_transfers = True
    ...
    def confirm_time_input(self):
        ...
        if valid:
            self.total_transfers = len(self.transfer_times) if self.choice == "custom" else 1
            self.current_transfer_number = 0
            self.handle_transfers()
        else:
            self.show_error_message("Invalid input! Please enter a whole number of hours between 1 and 23.")
    ...
    def handle_transfers(self):
        self.display_text_overlay_message()  # Display the overlay message
        self.create_execute_button()
    ...
    def start_transfers(self):
        ...
        transfer_msg = GUIMsg()
        transfer_msg.go_flag = "Execute"
        self.gohead_pub.publish(transfer_msg)
        self.log_message(f"Starting transfer at {datetime.now().strftime('%H:%M:%S')}")

    def gui_done_cb(self, msg):
        if msg.go_flag == "Done":
            self.log_message(f"Transfer completed at {datetime.now().strftime('%H:%M:%S')}")
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        app.update_idletasks()
        app.update()
        app.display_text_overlay_message() 
        rospy.sleep(0.05)